...
module.exports = function (app) {
    function Active () {
        this.getActive = function (cb) {
            request.get({
                url : app.get('asch address') + '/api/delegates/?orderBy=rate:asc&limit=101',
                json : true
        ...
        this.getForged = function (delegate, cb) {
            request.get({
                url : app.get('asch address') + '/api/delegates/forging/getForgedByAccount?generatorPublicKey=' + delegate.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
    ...
    this.getStandby = function (n, error, success) {
        ...
        request.get({
            url : app.get('asch address') + '/api/delegates/?orderBy=rate:asc' + '&limit='  + delegates.limit + '&offset=' + delegates.actualOffset,
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
    ...
    function Registrations () {
        this.getTransactions = function (cb) {
            request.get({
                url : app.get('asch address') + '/api/transactions/?orderBy=t_timestamp:desc&limit=5&type=2',
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getDelegate = function (tx, cb) {
            request.get({
                url : app.get('asch address') + '/api/delegates/get?publicKey=' + tx.senderPublicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
    ...
    function Votes () {
        this.getVotes = function (cb) {
            request.get({
                url : app.get('asch address') + '/api/transactions/?orderBy=t_timestamp:desc&limit=5&type=3',
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
    ...
    this.getNextForgers = function (error, success) {
        request.get({
            url : app.get('asch address') + '/api/delegates/getNextForgers?limit=101',
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
    ...
    this.getDelegateProposals = function (error, success) {
            var offset = 0,
                limit  = 25,
                nextPage = false,
                url = 'https://forum.asch.io/viewforum.php?f=48&start=',
    ...
    function LastBlock () {
        this.getBlock = function (cb) {
            request.get({
                url : app.get('asch address') + '/api/blocks?&orderBy=height:desc&limit=1',
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getDelegate = function (block, cb) {
            request.get({
                url : app.get('asch address') + '/api/delegates/get?publicKey=' + block.generatorPublicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
    ...
    this.getLastBlocks = function (params, error, success) {
        ...
        request.get({
            url : app.get('asch address') + '/api/blocks?orderBy=height:desc&generatorPublicKey=' + params.publicKey + '&limit=' + params.limit,
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : err });
    ...
    this.getSearch = function (params, error, success) {
           if (!params || !params.match(/^(?![0-9]{1,21}[L]$)[0-9a-z.]+/i)) {
            ...
            request.get({
                url : app.get('asch address') + '/api/delegates/search?q=' + params + '&limit=1',
                json : true
            }, function (err, response, body) {
