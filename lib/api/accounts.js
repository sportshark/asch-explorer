...
module.exports = function (app) {
    function Account () {
        ...
        this.getAccount = function (address, cb) {
            ...
            request.get({
                url : app.get('asch address') + '/api/accounts?address=' + address,
                json : true
        ...
        this.getDelegate = function (account, cb) {
            ...
            request.get({
                url : app.get('asch address') + '/api/delegates/get?publicKey=' + account.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getVotes = function (account, cb) {
            ...
            request.get({
                url : app.get('asch address') + '/api/accounts/delegates/?address=' + account.address,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getVoters = function (account, cb) {
            ...
            request.get({
                url : app.get('asch address') + '/api/delegates/voters?publicKey=' + account.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getIncomingTxsCnt = function (account, cb) {
            ...
            request.get({
                url : app.get('asch address') + '/api/transactions?recipientId=' + account.address + '&limit=1',
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getOutgoingTxsCnt = function (account, cb) {
            ...
            request.get({
                url : app.get('asch address') + '/api/transactions?senderId=' + account.address + '&limit=1',
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getForged = function (account, cb) {
            ...
            request.get({
                url : app.get('asch address') + '/api/delegates/forging/getForgedByAccount?generatorPublicKey=' + account.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
    ...
    this.getTopAccounts = function (query, error, success) {
        ...
        request.get({
            url : app.get('asch address') + '/api/accounts/top?&offset=' + param(query.offset, 0) + '&limit=' + param(query.limit, 100),
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
