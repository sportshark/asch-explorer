...
var Header = function ($rootScope) {
    ...
    this.updatePriceTicker = function (res) {
        ...
        // When ticker for user-stored currency is not available - switch to XAS temporarly
        if ($rootScope.currency.symbol !== 'XAS' && (!$rootScope.currency.tickers || !$rootScope.currency.tickers.XAS || !$rootScope.currency.tickers.XAS[$rootScope.currency.symbol])) {
            console.log ('Currency ' + $rootScope.currency.symbol + ' not available, fallback to XAS');
            $rootScope.currency.symbol = 'XAS';
        }
    };
...
angular.module('asch_explorer.system').factory('header',
