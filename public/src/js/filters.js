...
angular.module('asch_explorer')
  ...
  .filter('currency', function (numberFilter, aschFilter) {
      return function (amount, currency, decimal_places) {
        var asch = aschFilter (amount),
            factor = 1;

        if (currency.tickers && currency.tickers.XAS && currency.tickers.XAS[currency.symbol]) {
          factor = currency.tickers.XAS[currency.symbol];
        } else if (currency.symbol !== 'XAS') {
        ...
        if (decimal_places === undefined) {
          switch (currency.symbol) {
            case 'XAS':
            case 'BTC':
              return numberFilter ((asch * factor), 8).replace (/\.?0+$/, '');
            default:
              return numberFilter ((asch * factor), 2).replace (/\.?0+$/, '');
          }
        } else {
          return numberFilter ((asch * factor), decimal_places);
        }
      };
  })
  ...
  .filter('votes', function () {
      ...
      return function (name, proposals) {
          ...
          if (p) {
              return $sce.trustAsHtml('<a class="glyphicon glyphicon-user" href="https://forum.asch.io/viewtopic.php?f=48&t=' + p.topic + '" title="' + _.escape (p.description) + '" target="_blank"></a> ' + name);
          } else {
